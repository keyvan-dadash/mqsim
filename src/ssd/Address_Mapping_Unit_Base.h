...
namespace SSD_Components
{
	...
	public:
		...
		**********************************************************************************************************************/
		...
		virtual void Set_barrier_for_accessing_physical_superblock(const NVM::FlashMemory::Physical_Page_Address& superblock_address) = 0; //At the very beginning of executing a GC request, the GC target physical superblock (that is selected for erase) should be protected by a barrier. The LPAs within this superblock are unknown until the content of the physical pages within the superblock are read one-by-one. Therfore, at the start of the GC execution, the barrier is set for the physical superblock. Later, when the LPAs are read from the physical superblock, the above functions are used to lock each of the LPAs.
	...
	protected:
		...
		//virtual bool query_cmt(NVM_Transaction_Flash* transaction, const std::list<NVM_Transaction*>& transactionList) = 0;
