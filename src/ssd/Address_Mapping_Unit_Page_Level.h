...
#define MAKE_TABLE_INDEX(LPN,STREAM)
	...
	* its own share of CMT (equal partitioning of CMT space among concurrent streams).*/
	...
	public:
		...
		void Set_barrier_for_accessing_physical_superblock(const NVM::FlashMemory::Physical_Page_Address& superblock_address);
	...
	private:
		...
		void allocate_page_in_plane_for_user_write(NVM_Transaction_Flash_WR* transaction, bool is_for_gc, const std::list<NVM_Transaction*>& transactionList);
		...
		//bool translate_lpa_to_ppa_for_write_tr(stream_id_type streamID, const std::list<NVM_Transaction*>& transactionList);
		...
		//bool query_cmt(NVM_Transaction_Flash* transaction, const std::list<NVM_Transaction*>& transactionList);
