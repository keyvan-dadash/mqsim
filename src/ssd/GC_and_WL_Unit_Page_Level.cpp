...
namespace SSD_Components
{
	...
	void GC_and_WL_Unit_Page_Level::Check_gc_required_die(const unsigned int free_superblock_pool_size, const NVM::FlashMemory::Physical_Page_Address& die_address){
		//----------------------------------------------- DEBUG BY MAEE ---------------------------------------------------------------
		bool target_plane_debug_enabled = (die_address.ChannelID == target_debug_channel && die_address.ChipID == target_debug_chip && die_address.DieID == target_debug_die);
		if (target_plane_debug_enabled) {
			_debug_by_maee(
				std::ofstream debug_file;
			debug_file.open("debug_file.txt", std::ios_base::app);
			debug_file << "free super block pool size: " << free_superblock_pool_size << " , block pool gc threshold: " << block_pool_gc_threshold << std::endl;
			debug_file.close();
			)
		}
		//-----------------------------------------------------------------------------------------------------------------------------
		...
		if (free_superblock_pool_size < block_pool_gc_threshold) {
			...
			if (target_plane_debug_enabled) {
				_debug_by_maee(
					std::ofstream debug_file;
					debug_file.open("debug_file.txt", std::ios_base::app);
					debug_file << "GC is required" << std::endl;
					debug_file.close();
				)
			}
			//-----------------------------------------------------------------------------------------------------------------------------
			...
			if (target_plane_debug_enabled) {
				_debug_by_maee(
					std::ofstream debug_file;
					debug_file.open("debug_file.txt", std::ios_base::app);
					debug_file << "candidate gc superblock is: " << gc_candidate_superblock_id << std::endl;
					debug_file.close();
				)
			}
			//DEBUG_MAEE(die_address.ChannelID << ", " << die_address.ChipID << ", " << die_address.DieID << ", " << die_address.PlaneID)
			//-----------------------------------------------------------------------------------------------------------------------------
			...
			gc_candidate_address.BlockID = gc_candidate_superblock_id;
			...
			if (target_plane_debug_enabled) {
				_debug_by_maee(
					std::ofstream debug_file;
					debug_file.open("debug_file.txt", std::ios_base::app);
					debug_file << "number of invalid pages of the first block of superblock : " << superblock->SuperblockID << " is: "
						<< superblock->Blocks[0]->Invalid_page_count << std::endl;
					debug_file << "invalid pages bitmap of the first block of superblock : " << superblock->SuperblockID << " is: "
						<< superblock->Blocks[0]->Invalid_page_bitmap << std::endl;
			...
			for (unsigned int i = 0; i < superblock->block_no_per_superblock; i++) {
				gc_candidate_address.PlaneID = i;
			...
			for (unsigned int i = 0; i < superblock->block_no_per_superblock; i++) {
				gc_candidate_address.PlaneID = i;
				address_mapping_unit->Set_barrier_for_accessing_physical_block(gc_candidate_address);//Lock the blocks of superblock, so no user request can intervene while the GC is progressing
			}
			...
			if (block_manager->Can_execute_gc_wl_die(gc_candidate_address)) {
				for (unsigned int i = 0; i < superblock->block_no_per_superblock; i++){
					gc_candidate_address.PlaneID = i;
					dbke->plane_manager_die[i].Ongoing_erase_operations.insert(gc_candidate_superblock_id);
