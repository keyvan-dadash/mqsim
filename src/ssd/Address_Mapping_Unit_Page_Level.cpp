...
namespace SSD_Components
{
	...
	{
		//std::list<NVM_Transaction*>::const_iterator it;
		//for (it = transactionList.begin(); it != transactionList.end(); ) {
		...
		//	} else {
		//		if (((NVM_Transaction_Flash*)(*it))->Type == Transaction_Type::READ) {
		...
		//	query_cmt((NVM_Transaction_Flash*)(*it++), transactionList);
		//}
	...
	//bool Address_Mapping_Unit_Page_Level::query_cmt(NVM_Transaction_Flash* transaction, const std::list<NVM_Transaction*>& transactionList)
	//{
	//	stream_id_type stream_id = transaction->Stream_id;
	...
	//			if (domains[stream_id]->Mapping_entry_accessible(ideal_mapping_table, stream_id, ((NVM_Transaction_Flash*)(*it))->LPA))//Either limited or unlimited CMT
	//			{
	//				Stats::CMT_hits_per_stream[stream_id]++;
	//				Stats::CMT_hits++;
	//				//This is a write transaction
	...
	//			}
	//		}
	//		if (translate_lpa_to_ppa_for_write_tr(stream_id, transactionList)) {
	//			return true;
	//		}
	//		return false;
	//	}
	...
	*/
	//bool Address_Mapping_Unit_Page_Level::translate_lpa_to_ppa_for_write_tr(stream_id_type streamID, const std::list<NVM_Transaction*>& transactionList)
	//{
	//	for (std::list<NVM_Transaction*>::const_iterator transaction = transactionList.begin(); transaction != transactionList.end(); ) {
	//		allocate_plane_for_user_write((NVM_Transaction_Flash_WR*)(*transaction));
	//		//there are too few free pages remaining only for GC
	//		if (ftl->GC_and_WL_Unit->Stop_servicing_writes(((NVM_Transaction_Flash_WR*)(*transaction))->Address)) {
	//			mange_unsuccessful_translation(((NVM_Transaction_Flash_WR*)(*transaction)));
	//			return false;
	//		}
	//	}
	...
	bool Address_Mapping_Unit_Page_Level::translate_lpa_to_ppa(stream_id_type streamID, NVM_Transaction_Flash* transaction)
	{
		...
		if (transaction->Type == Transaction_Type::READ) {
			...
			if (ftl->GC_and_WL_Unit->Stop_servicing_writes(transaction->Address)){
				//DEBUG_MAEE("Stop Servicing Writes.")
				return false;
			}
			allocate_page_in_plane_for_user_write((NVM_Transaction_Flash_WR*)transaction, false);
	...
	void Address_Mapping_Unit_Page_Level::allocate_page_in_plane_for_user_write(NVM_Transaction_Flash_WR* transaction, bool is_for_gc, const std::list<NVM_Transaction*>& transactionList)
	{
		AddressMappingDomain* domain = domains[transaction->Stream_id];
	...
	inline void Address_Mapping_Unit_Page_Level::Set_barrier_for_accessing_physical_block(const NVM::FlashMemory::Physical_Page_Address& block_address)
	{
		...
		Block_Pool_Slot_Type* block = &(block_manager->die_manager[block_address.ChannelID][block_address.ChipID][block_address.DieID].plane_manager_die[block_address.PlaneID].Blocks[block_address.BlockID]);
	...
	inline void Address_Mapping_Unit_Page_Level::Set_barrier_for_accessing_physical_superblock(const NVM::FlashMemory::Physical_Page_Address& superblock_address)
	{
		...
		for (flash_block_ID_type block_id = 0; block_id < superblock->block_no_per_superblock; block_id++){
			...
			for (flash_page_ID_type pageID = 0; pageID < block->Current_page_write_index; pageID++) {
				if (block_manager->Is_page_valid(block, pageID)) {
					addr.PlaneID = block_id;
					addr.BlockID = block->BlockID;
					addr.PageID = pageID;
					if (block->Holds_mapping_data) {
						MVPN_type mpvn = (MVPN_type)flash_controller->Get_metadata(addr.ChannelID, addr.ChipID, addr.DieID, addr.PlaneID, addr.BlockID, addr.PageID);
						if (domains[block->Stream_id]->GlobalTranslationDirectory[mpvn].MPPN != Convert_address_to_ppa(addr)) {
