...
namespace SSD_Components
{
	...
	void GC_and_WL_Unit_Base::handle_transaction_serviced_signal_from_PHY(NVM_Transaction_Flash* transaction)
	{
		...
		switch (transaction->Type) {
			...
			case Transaction_Type::ERASE:
				...
				//-------------------------- ADDED BY MAEE ---------------------------------------------------
				_my_instance->block_manager->check_if_a_superblock_erased(transaction->Address);
				//--------------------------------------------------------------------------------------------
				...
				if (_my_instance->Stop_servicing_writes(transaction->Address)) {
					_my_instance->Check_gc_required_die(pbke->Get_free_block_pool_size(), transaction->Address);
				}
				break;
			} //switch (transaction->Type)
	}
	...
	bool GC_and_WL_Unit_Base::Stop_servicing_writes(const NVM::FlashMemory::Physical_Page_Address& plane_address)
	{
		...
		//----------------------------------------------- DEBUG BY MAEE ---------------------------------------------------------------
		bool target_plane_debug_enabled = (plane_address.ChannelID == target_debug_channel && plane_address.ChipID == target_debug_chip && plane_address.DieID == target_debug_die);
		if (target_plane_debug_enabled) {
			_debug_by_maee(
				std::ofstream debug_file;
				debug_file.open("debug_file.txt", std::ios_base::app);
				debug_file << "Free block pool size of plane " << plane_address.ChannelID << ", " << plane_address.ChipID <<
					", " << plane_address.DieID << ", " << plane_address.PlaneID << " is: " << block_manager->Get_pool_size(plane_address) << std::endl;
				if (block_manager->Get_pool_size(plane_address) < max_ongoing_gc_reqs_per_plane) {
					debug_file << "Stop servicing writes." << std::endl;
				}
				debug_file.close();
			)
		}
	...
	bool GC_and_WL_Unit_Base::is_safe_gc_wl_candidate_superblock(const DieBookKeepingType* die_record, const flash_superblock_ID_type gc_wl_candidate_superblock_id)
	{
		...
		for (int i = 0; i < plane_no_per_die; i++) {
			if (die_record->plane_manager_die[i].Blocks[gc_wl_candidate_superblock_id].Ongoing_user_program_count > 0) {
				return false;
			}
		}
