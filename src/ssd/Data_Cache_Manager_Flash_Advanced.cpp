	...
	void Data_Cache_Manager_Flash_Advanced::process_new_user_request(User_Request* user_request)
	{
		...
		else//This is a write request
		{
			...
			/*_debug_by_maee(
				...
				debug_file << "A new Write request of user " << user_request->ID << "." << std::endl;
				debug_file.close();
			)*/
			//--------------------------------------------------------------------------------------------------------------------
			...
			switch (caching_mode_per_input_stream[user_request->Stream_id])
			{
				...
				case Caching_Mode::READ_CACHE:
					...
					_analyze_by_maee(
						...
						analyze_file.open("analyze_file.txt", std::ios_base::app);
						analyze_file << "Request " << user_request->ID << ": " << std::endl;
						...
						for (std::list<NVM_Transaction*>::const_iterator it = user_request->Transaction_list.begin(); it != user_request->Transaction_list.end(); it++) {
							analyze_file << "transaction #" << i++ << " LPA: " << ((NVM_Transaction_Flash*)(*it))->LPA << " --- Address: " << ((NVM_Transaction_Flash*)(*it))->Address.ChannelID
								<< ", " << ((NVM_Transaction_Flash*)(*it))->Address.ChipID << ", " << ((NVM_Transaction_Flash*)(*it))->Address.DieID << std::endl;
						}
						analyze_file.close();
					)
						//---------------------------------------------------------------------------------------------------------
